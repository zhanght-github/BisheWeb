         ...
         <el-table-column
          ...
          width="240">
            <template slot-scope="scope">
                <div class="handle">
                  <span v-if='scope.row.paperfinalPath' class="allowed result">定稿已提交</span>
                  <span v-if='scope.row.paperfinalPath' class="watchSuggest" @click="openPaper(scope.row)">查看论文</span>
                  <span v-if='!scope.row.paperfinalPath' class="notAllowed result">定稿未提交</span>
                </div>
            </template>
          </el-table-column>
          <el-table-column
          ...
          width="240">
            <template slot-scope="scope">
              <div v-if='!scope.row.paperfinalIspass' class="handle">
                <span class="notAllowed result">审阅老师暂未审阅</span>
              </div>
              <div v-if='scope.row.paperfinalIspass == 1' class="handle">
...
<script>
import { topicInfo, defencelist, finalispass } from '@/api/teacher'
...
export default {
  ...
  methods: {
    getData() {
      defencelist(this.pageNum - 1, this.pageSize, this.getUserId()).then(res => {
        ...
        this.datalist.forEach(item => {
          if (item.paperfinalPath) {
            item.url = CMethods.spliceDownloadUrl(item.topicname, item.paperfinalPath)
          }
        })
      })
    },
    openPaper(val) {
      ...
      if (val.paperfinalPath) {
        studentAssign(val.studentid).then(res => {
          var ids = res.data.data.topicid
          topicInfo(ids).then(res => {
            res.data.data.url = CMethods.spliceDownloadUrl(res.data.data.topicname, val.paperfinalPath)
    ...
    openSuggest(val) {
      if (val.paperfinalSuggest) {
        this.$confirm(`${val.paperfinalSuggest}`, '审核意见', {
    ...
    review() {
      ...
      finalispass(this.updataData.studentid, this.updataData.ispass, this.updataData.suggest).then(res => {
        this.Dialog = false
        this.reset()
        this.getData()
      })
