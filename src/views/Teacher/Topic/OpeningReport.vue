         ...
         <el-table-column
          ...
          label="文件">
            <template slot-scope="scope">
              <a  class="clickdoc doclabel" v-if='scope.row.openreportPath' :href="scope.row.url">{{scope.row.openreportPath}}</a>
              <a  class="doclabel" v-if='!scope.row.openreportPath' href="">未上传开题报告</a>
            </template>
          </el-table-column>
          <el-table-column
          label="审核状态">
          <template slot-scope="scope">
              <div v-if='scope.row.openreportIspass == 0' class="handle">
                <span class="notAllowed result">未审核</span >
              </div>
              <div v-if='scope.row.openreportIspass == 1' class="handle">
                <span class="allowed result">{{scope.row.openreportScore}}分已通过</span >
                <!-- <el-button type="primary" plain class="orangebtn" >撤销选题</el-button> -->
              </div>
              <div v-if='scope.row.openreportIspass == -1' class="handle">
                <span class="low result">{{scope.row.openreportScore}}分未通过</span >
                <!-- <el-button type="primary" plain class="orangebtn" >撤销选题</el-button> -->
              </div>
            </template>
          </el-table-column>
          ...
          width="240">
            <template slot-scope="scope">
              <div class="handle">
                <el-button v-if='scope.row.openreportIspass == 0' type="primary" plain class="deepbluebtn" @click="openDialog(scope.row)">评阅</el-button>
                <el-button v-if='scope.row.openreportIspass != 0' type="primary" plain class="deepbluebtn" @click="openDialog(scope.row)">修改</el-button>
              </div>
            </template>
          </el-table-column>
    </el-table>
...
import { getOpenlist, openingispass } from '@/api/teacher'
    ...
    getData() {
      getOpenlist(this.pageNum - 1, this.pageSize, this.getUserId()).then(res => {
        ...
        this.dataList.forEach(item => {
          item.url = CMethods.spliceDownloadUrl(item.topicname, item.openreportPath)
        })
      })
    },
    openDialog(val) {
